...
When initializing Firebase Auth on RN you **must** use `initializeAuth` with `getReactNativePersistence(AsyncStorage)`, not `getAuth`, or users will be logged out on every app restart. The TypeScript types for `getReactNativePersistence` are missing (firebase-js-sdk#5677) — the existing `lib/firebase/index.ts` works around it with a localized `@ts-expect-error`; don't "fix" that comment unless the upstream types ship.

**Firestore offline persistence on the JS SDK + RN is memory-only.** The JS SDK's `persistentLocalCache` uses IndexedDB, which doesn't exist on React Native. The in-memory write queue *does* sync writes when network returns within a session, but writes do **not** survive an app restart with no network. Real persistent offline-first arrives at Milestone 7 alongside the React Native Firebase migration. If we see real users losing check-ins before then, build a custom AsyncStorage write-queue layer above the Firestore client — don't add any other "offline" hack.
